...
namespace N2.Core.BaseProvider
{
    public abstract class RepositoryBase<TEntity> where TEntity : BaseEntity, new()
    {
        public RepositoryBase(N2Context dbContext)
        {
            this.DefaultDbContext = dbContext;
        }

        private N2Context DefaultDbContext { get; set; }
        public N2Context N2Context
        {
            get
            {
        ...
        {
            get { return N2Context.Set<TEntity>(); }
        }
        ...
        {
            return N2Context.Set<TExists>(filterDeleted).Any(predicate);
        }
        ...
        public virtual ISugarQueryable<TEntity> WhereIF(bool checkCondition, Expression<Func<TEntity, bool>> predicate)
        {
            if (checkCondition)
            {
                return N2Context.Set<TEntity>().Where(predicate);
            }
            return N2Context.Set<TEntity>();
        }
        ...
        public virtual ISugarQueryable<T> WhereIF<T>(bool checkCondition, Expression<Func<T, bool>> predicate) where T : class
        {
            if (checkCondition)
            {
                return N2Context.Set<T>().Where(predicate);
            }
            return N2Context.Set<T>();
        }


        public virtual List<TFind> Find<TFind>(Expression<Func<TFind, bool>> predicate, bool filterDeleted = true) where TFind : class
        {
            return N2Context.Set<TFind>(filterDeleted).Where(predicate).ToList();
        }
        ...
        {
            return N2Context.Set<TFind>(filterDeleted).Where(predicate);
        }
        ...
        {
            return N2Context.Set<TEntity>(filterDeleted).Where(predicate).Select(selector).ToList();
        }
        /// <summary>
        /// 单表查询
        /// </summary>
        ...
        {
            return N2Context.Set<TEntity>(filterDeleted).Where(predicate).FirstOrDefault();
        }
        ...
        {
            return N2Context.SaveChanges();
        }

        public virtual Task<int> SaveChangesAsync()
        {
            return N2Context.SqlSugarClient.SaveChangesAsync();
        }
